...
Ein LSTM ist eine bestimmte Form der RNNs. Ein LSTM ist so gebaut, dass es die einzelnen Units selber entscheiden können, welche Daten weiter beibehalten oder vergessen werden - es hat einen eigenen Speicher, dessen Benutzung trainiert werden kann. Durch diese dynamische Speicherverwaltung, kann es auf der einen Seite sich bestimmte Informationen länger speichern, auf der anderen, kurzfristig benötigte Daten verarbeiten und dann wieder vergessen. Während des Trainings eines LSTMs, erlernt dieses auch das Speichern und Löschen. Dadurch kann es sehr viel effizienter als reine RNNs Daten mit temporaler Dimension auswerten. Fast jeder Erfolg, der mit RNNs erzielt wurde, ist auf LSTMs zurückzuführen \cite{colahsBlogLSTM}. 
...
\end{figure}
Trotz der geringeren Anzahl an Parameter, ist die Performance ähnlich der klassische Variante\cite{lstmSearchSpace}, trainiert aber schneller, da weniger optimiert werden muss.  Im LSTM erfolgt eine Kontrolle darüber, wie viel des Cell-States mit in einer Berechnung einfließt. Im GRU auf der andern Seite, wird immer die volle Historie mit in die Berechnung aufgenommen \cite{paperGRUComparison}. 
...
Um den Code für die Website Screenshots zu generieren, wird ein Modell bestehend aus 2 Rekurrenten Netzwerken und einem Convolutional Neural Network erstellt. Das hier benutzte Modell ist eine Abwandlung des im Original Paper verwendeten Modells. Da die neue DSL komplexer als die im Original Paper\cite{Beltramelli17} ist, musste sowohl das CNN als auch die RNNs verändert werden. 
...
Das erste RNN, wird dafür benötigt, die DSL zu lernen, insbesondere die validen Reihenfolgen und die Grammatik. Das zweite RNN, bekommt als Input, den Output des CNN, also eine Repräsentation des Bildes sowie den Output des ersten RNNs. Zu sehen ist dieses Zusammenspiel in der Abbildung~\ref{fig:pix2code_paper_architecture}.
...
Danach werden 2 Fully-Connected Ebenen (\texttt{Dense} im Code) genutzt. Danach wird der Output über den \texttt{RepeatVector} an das Decoder-RNN weitergegeben.

\subsection{Sprach Modell}

Da für das Problem der Screenshot-Erstellung eine einfache Sprache erstellt wurde, kann jeder der Token über ein One-Hot-Encoding modelliert werden. Eine Sequenz von Token, wird nun ein Sequenz von Ont-Hot-Encodings, die so in das Sprach-Modell gegeben werden. Das Sprach-Modell lernt aus diesen Sequenzen eine eigene, Modell-interne Repräsentation der Sprache. Diese Repräsentation wird dem Output des Vision-Modells angehängt und in den Decoder Teil des Netzwerkes gegeben. 

Ein klassisches RNN würde bei dem Task der Sprach-Modellierung schlechter abschneiden, da es in der hier benutzen DSL lange Abhängigkeiten gibt \cite{colahsBlogLSTM}. Deswegen besteht das Sprach-Modell im Original Paper aus zwei LSTM-Ebenen mit jeweils 128 Units. In dieser Arbeit wurde jedoch bessere Ergebnisse mit GRU-Ebenen erstellt. Beides sind erweiterte Varianten von RNNs, und unterscheiden sich hauptsächlich in der Art wie der Modell-interne Speicher verwaltet wird \cite{colahsBlogLSTM}. 

\subsection{Decoder Modell}

Der Decoder-Teil des generiert aus dem zusammengefügten Inputs des Vision und des Sprach Modells die Token-Sequenz.
Es besteht im Original Paper aus zwei LSTM-Ebenen mit jeweils 512 Units. Auch hier konnten bessere Ergebnisse mit GRUs erzeugt werden. Wie in Abbildung~\ref{fig:pix2code_paper_architecture} zu sehen ist, wird so lange durch das gesamte Netzwerk gelooped, bis dieses den speziellen \texttt{End}-Token generiert.
...
Durch die Verkettung der jeweiligen Netzwerke ist die Gesamtarchitektur nach wie vor ableitbar, und damit von Anfang bis Ende optimierbar. Was außerdem die Komplexität des Netzwerkes verringert, ist die geringe Token Anzahl. Durch das One-Hot-Encoding jedes Tokens, ist die Aufgabe des Netzwerkes ein reines Problem der Klassifizierung.
